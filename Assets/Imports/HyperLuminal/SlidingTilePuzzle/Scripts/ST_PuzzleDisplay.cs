...
{
	// this puzzle texture.
	public Texture[] PuzzleImages = new Texture[8];
	...
	void Awake () 
	{
		...
		this.transform.localPosition += Vector3.forward * 3; // hide
		// Positioning Tiles happen on Awake() of this children. by Coroutine.
		// so Setting off this Activation interrupt tiles positioning.
		// I just hide this while positioning.
	...
	private void ProcessComplete()
    {
		...
		completeMesh.gameObject.SetActive(true);

		Debug.Log(PathFinder.Instance.CurrentCell.Item1 + " " + PathFinder.Instance.CurrentCell.Item2);
		int rand = Random.Range(0, 11);
		
		if(rand % 2 == 0)
			PathFinder.Instance.FindPath();

		//Destroy(rewardCanvas, 5f);
		Destroy(keypad.gameObject);
		Destroy(gameObject);
	}
	...
	private void CreatePuzzleTiles()
	{
		...
		for (int j = Height - 1; j >= 0; j--)
		{
			for(int i = 0; i < Width; i++)
			{
				...
				thisTileMaterial.mainTexture = PuzzleImages[rand];
	...
	private ST_PuzzleTile MappingGrid2Display(int x, int y)
    {
		ST_PuzzleTile mappedTile = null;
		for (int j = Height - 1; j >= 0; j--)
		{
			for (int i = 0; i < Width; i++)
			{
				...
				if ((TileDisplayArray[i, j].GetComponent<ST_PuzzleTile>().GridLocation.x == x) &&
				   (TileDisplayArray[i, j].GetComponent<ST_PuzzleTile>().GridLocation.y == y))
				{
						mappedTile = TileDisplayArray[i, j].GetComponent<ST_PuzzleTile>();
				}
			}
		}
	...
	public void MoveTile(int idx)
    {
		Vector2 gridIndex = ConvertIndexToGrid(idx);

		Debug.Log(idx + " " + gridIndex.x + " " + gridIndex.y);

		ST_PuzzleTile mapped = MappingGrid2Display((int)gridIndex.x, (int)gridIndex.y);
		Debug.Log(mapped.GridLocation.x + " " + mapped.GridLocation.y);

		mapped.ExecuteAdditionalMove();
	}
}
