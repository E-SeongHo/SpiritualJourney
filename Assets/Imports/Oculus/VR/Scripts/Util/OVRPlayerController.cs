...
public class OVRPlayerController : MonoBehaviour
{
    ...
    public virtual void UpdateMovement()
    {
        ...
        if (EnableLinearMovement)
        {
            ...
            float run = OVRInput.Get(OVRInput.Axis1D.PrimaryIndexTrigger);
            if (run > 0.0f) Running = true;
            else Running = false;

            moveInfluence *= 1.0f + run;
#endif

            float run = OVRInput.Get(OVRInput.Axis1D.SecondaryIndexTrigger);
            if (run > 0.0f) Running = true;
            else Running = false;
